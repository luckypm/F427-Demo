...
static int32_t filerProcessStream(filerFileStruct_t *f, uint8_t final) {
	...
	res = f_write(&f->fp, (char*)(f->buf) + f->tail, size, &bytes);//keil中要在编绎器中加-gnu才能编绎通过，不知为什么
...
void filerTaskCode(void *p) {
	...
	for (i = 0; i < FILER_MAX_FILES; i++) {
		...
		if (filerData.files[i].function == FILER_FUNC_STREAM) {
			...
			filerProcessSync(&filerData.files[i]);//1秒进行一次sync，防止异常
		    }
		}
	    }
	}
	utilTaskPeriodTime(filerData.timer);
	/* 执行周期200Hz */
	yield(5);
...
void filerInit(void) {
  ...
  //  filerData.filerFlag = CoCreateFlag(0, 0);   // manual reset
 	xTaskCreate((TaskFunction_t )filerTaskCode,     	
                (const char*    )"filer_task",   	
                (uint16_t       )FILER_STACK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )FILER_PRIORITY,	
                (TaskHandle_t*  )&filerTaskHandle);  
