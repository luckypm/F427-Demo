...
#define AQ_NOTICE		commNotice//最长512字节
#define AQ_PRINTF(fmt, args...)	{char *sTemp = commGetNoticeBuf(); snprintf(sTemp, COMM_NOTICE_LENGTH, fmt, args); commNotice(sTemp);}//字符串最长64字节

enum commStreamTypes {
    COMM_STREAM_TYPE_NONE	    = 0,// 0
    COMM_STREAM_TYPE_MULTIPLEX	    = (1<<0),// 2^0
    COMM_STREAM_TYPE_MAVLINK	    = (1<<1),// 2^1
    COMM_STREAM_TYPE_TELEMETRY	    = (1<<2),// 2^2
    COMM_STREAM_TYPE_GPS	    = (1<<3),// 2^3
    COMM_STREAM_TYPE_FILEIO	    = (1<<4),// 2^4
    COMM_STREAM_TYPE_CLI	    = (1<<5),// 2^5
    COMM_STREAM_TYPE_OMAP_CONSOLE   = (1<<6),// 2^6
    COMM_STREAM_TYPE_OMAP_PPP	    = (1<<7)// 2^7
};
...
#define COMM_HEADER_SIZE	9//8			    // TODO: hard-coded for now 这里应该是9，header是commTxBuf,除buf外还占了9个字节
