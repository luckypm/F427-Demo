...
void commNotice(const char *s) {
    ...
    if (commData.initialized)	
		xQueueSend(commData.noticesQueue,&s,0);
...
commTxBuf_t *commGetTxBuf(uint8_t streamType, uint16_t maxSize) {
    ...
    if (commData.typesUsed & streamType) {
        ...
        xSemaphoreTake(commData.txBufferMutex,portMAX_DELAY);
        ...
        if (i < COMM_TX_NUM_SIZES) {
            ...
            for (j = 0; j < commData.txPacketBufNum[i]; j++) {
                tmp = (commTxBuf_t*)((char*)(commData.txPacketBufs[i]) + (commData.txPacketBufSizes[i] + COMM_HEADER_SIZE) * j);
            ...
            // need an upgrade? 这个BUFF块里没有空闲BUFF时，就去下一个BUFF块里寻找
        ...
        xSemaphoreGive(commData.txBufferMutex);
...
void commTxFinished(void *param) {
    ...
    //if (__sync_sub_and_fetch(&txBuf->status, 1) == COMM_TX_BUF_SENDING)//__sync_sub_and_fetch是GCC提供的一种原子操作函数，先减再返回
    ...
    if (txBuf) {
        ...
        for (i = 0; i < COMM_NUM_PORTS; i++) {
            ...
            if (commData.portStreams[i] == txBuf->type && commData.portTypes[i] != COMM_PORT_TYPE_USB && commData.portTypes[i] != COMM_PORT_TYPE_NONE) {
                ...
                // check for stack overruns 检查栈是否溢出
...
static void commCheckNotices(void) {
    ...
    char *s;
	char head[10];
	result = xQueueReceive(commData.noticesQueue,&s,0);
    ...
    if (result == pdTRUE) {
        ...
        i = j = 0;
		//加上一个时间头
		sprintf(head,"%d:%02d> ",(timerMicros()/1000000)/60,(timerMicros()/1000000)%60);
		while(head[j] != 0){
			 commLog[commData.logPointer] = head[j];
...
void commTaskCode(void *unused) {
    ...
    while (1) {
        ...
        //canUartStream();
        loops++;
		utilTaskPeriodTime(commData.timer);
    }
}
...
void commInit(void) {
    ...
    commData.portStreams[2] = (uint8_t)p[COMM_STREAM_TYP3];//TYP3默认为0
    ...
    commData.portStreams[3] = (uint8_t)p[COMM_STREAM_TYP4];//TYP4默认为0
    commData.portTypes[3] = COMM_PORT_TYPE_SERIAL;
#endif

    // record which stream types that we are working with记录所有comm_port口所用的streamtype
    commSetTypesUsed();
    ...
    // allocate transmission buffers' memory 实际分配的大小是比bufSize大了一个head,用来存储head
    for (i = 0; i < COMM_TX_NUM_SIZES; i++)
    ...
    // setup mutex
    commData.txBufferMutex = xSemaphoreCreateMutex();

    // notice queue 创建一个可以存放16个消息指针的消息队列
    commData.noticesQueue = xQueueCreate(COMM_NOTICE_DEPTH,sizeof(char *)); 
    xTaskCreate((TaskFunction_t )commTaskCode,     	
                (const char*    )"comm_task",   	
                (uint16_t       )COMM_STACK_SIZE, 
                (void*          )NULL,				
                (UBaseType_t    )COMM_PRIORITY,	
                (TaskHandle_t*  )&commTaskHandle);  

#ifdef COMM_USB_PORT
    usbInit();
    commData.portTypes[COMM_USB_PORT] = COMM_PORT_TYPE_USB;
	//commData.portStreams[COMM_USB_PORT] = COMM_STREAM_TYPE_TELEMETRY;
#endif
