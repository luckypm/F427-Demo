...
static void ubloxSetTimepulse(void) {
    ...
    ubloxWriteU4(1000000);              // interval (us)脉冲周期1s
    ubloxWriteU4(100000);               // length (us)脉宽0.1s 即占空比10%
#ifdef GPS_LATENCY
    ubloxWriteI1(0x00);                 // config setting (0 == off)AQ中是关闭了时钟脉冲 ？
#else
...
void ubloxInitGps(void) {
    ...
    ubloxWriteU2(0x0007);           // inProtoMask输入支持ubx NMEA RTCM协议
    ubloxWriteU2(0x0001);           // outProtoMask输出支持ubx协议
...
void ubloxSendSetup(void) {
    ...
    ubloxEnableMessage(UBLOX_AID_CLASS, UBLOX_AID_REQ, 1);      // AID REQ M8中已经没有这条消息了
    ubloxEnableMessage(UBLOX_NAV_CLASS, UBLOX_NAV_TIMEUTC, 5);  // NAV TIMEUTC
#ifdef GPS_DO_RTK
    ubloxEnableMessage(UBLOX_RXM_CLASS, UBLOX_RXM_RAW, 1);      // RXM RAW	M8中已经没有这条消息了
...
unsigned char ubloxPublish(void) {
    ...
    vTaskPrioritySet(gpsTaskHandle, configMAX_PRIORITIES-1);
...
#ifdef GPS_LATENCY
            gpsData.lastPosUpdate = timerMicros() - GPS_LATENCY;//GPS上一次更新时间=现在时间-GPS的延迟时间
#else
    ...
    vTaskPrioritySet(gpsTaskHandle,GPS_PRIORITY);
    ...
    // TODO
    if (0 && commStreamUsed(COMM_STREAM_TYPE_TELEMETRY)) {
        commTxBuf_t *txBuf;
        uint8_t *ptr;
        int i;
