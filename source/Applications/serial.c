...
serialPort_t *serialUSART5(unsigned int flowControl, unsigned int rxBufSize, unsigned int txBufSize) {
    GPIO_InitTypeDef GPIO_InitStructure;
    //NVIC_InitTypeDef NVIC_InitStructure;//因未用到，为避免警告，暂时注释掉
    serialPort_t *s;
...
serialPort_t *serialOpen(USART_TypeDef *USARTx, unsigned int baud, uint16_t flowControl, unsigned int rxBufSize, unsigned int txBufSize) {
	...
	s->rxPos = DMA_GetCurrDataCounter(s->rxDMAStream);//初始化为s->rxBufSize
    }
    // otherwise use ISR
    else {
    ...
    // Configure DMA for tx tx发送完是有中断
    if (s->txDMAStream) {
	DMA_DeInit(s->txDMAStream);
	DMA_InitStructure.DMA_Channel = s->txDMAChannel;
	DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
	DMA_InitStructure.DMA_BufferSize = (s->txBufSize != 0) ? s->txBufSize : 16;//后续会变
...
void serialPrint(serialPort_t *s, const char *str) {//可以用来向串口输出字符串
    while (*str)
	serialWrite(s, *(str++));
}
