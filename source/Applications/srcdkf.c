...
// s m v n 的值在初始化时分别是17    3      12     3(这些注释都是针对navUkf来说的，Alt_ukf不一样)
srcdkf_t *srcdkfInit(int s, int m, int v, int n, SRCDKFTimeUpdate_t *timeUpdate) {
	srcdkf_t *f;
	int maxN = MAX(v, n);//求出v n中的最大值=12
	...
	matrixInit(&f->Sx, s, s);//矩阵初始化，几行几列，矩阵的元素都赋为0
	matrixInit(&f->SxT, s, s);
	matrixInit(&f->Sv, v, v);
	matrixInit(&f->Sn, n, n);
	matrixInit(&f->x, s, 1);//状态估计矩阵是一个s行1列的列向量，即17行1列
	matrixInit(&f->Xa, s+maxN, 1+(s+maxN)*2);

	matrixInit(&f->qrTempS, s, (s+v)*2);
	matrixInit(&f->y, m, 1);//测量估计矩阵是一个m行1列的列向量，即3行1列
	...
	matrixInit(&f->K, s, m);
	matrixInit(&f->inov, m, 1);//inov矩阵是一个3行1列的列向量
	matrixInit(&f->xUpdate, s, 1);//xUpdate是一个17行1列的列向量
	...
	f->h = SRCDKF_H;//h值为3根号3
	f->hh = f->h*f->h;//h的平方=27
//	f->w0m = (f->hh - (float32_t)s) / f->hh;	// calculated in process
	...
	f->wic2 = __sqrtf((f->hh - 1.0f) / (4.0f * f->hh*f->hh));
        ...
        f->timeUpdate = timeUpdate;//初始化函数指针f->timeUpdate，让其地址与timpUpdate一样，即指向navUkfTimeUpdate()函数
...
static void srcdkfCalcSigmaPoints(srcdkf_t *f, arm_matrix_instance_f32 *Sn) {
	int S = f->S;			// number of states//初始化时为17
	int N = Sn->numRows;		// number of noise variables//初始化时为3
	int A = S+N;			// number of agumented states//扩展的状态量? A=17+3=20
	int L = 1+A*2;			// number of sigma points//sigma点的数量L=41
	...
	// resize output matrix//Xa矩阵为20行41列，总共20*41=820个元素
	f->Xa.numRows = A;
	f->Xa.numCols = L;
	...
	//
	for (i = 0; i < A; i++) {
		int rOffset = i*L; // rOffset=i*41;
		float32_t base = (i < S) ? x[i] : 0.0f;//i<17时，base=x[i],i>17时，base=0;

		Xa[rOffset + 0] = base;//Xa所指向的扩展sigma点矩阵的第一列的元素用base赋值，这一列的前17个元素是用状态估计值赋值的，后3个元素为0值

		for (j = 1; j <= A; j++) {
			float32_t t = 0.0f;

			if (i < S && j < S+1)
				t = Sx[i*S + (j-1)]*f->h;//与状态协方差有关，f->h初始化为3根号3

			if (i >= S && j >= S+1)
				t = Sn->pData[(i-S)*N + (j-S-1)]*f->h;//与过程噪声有关，f->h初始化为3根号3

			Xa[rOffset + j]     = base + t;//Xa所指向的扩展sigma点矩阵的第2列到第21列用base+t赋值；
			Xa[rOffset + j + A] = base - t;//Xa所指向的扩展sigma点矩阵的第22列到第41列用base-t赋值；
		}
	}
}

void srcdkfTimeUpdate(srcdkf_t *f, float32_t *u, float32_t dt) {
	int S = f->S;			// number of states//初始化的S为17
	int V = f->V;			// number of noise variables//初始化的V为12
	int L;				// number of sigma points//sigma点数
	float32_t *x = f->x.pData;	// state estimate
	float32_t *Xa = f->Xa.pData;	// augmented sigma points//扩展有sigma点?
...
//			Xa[j*L + i] = xOut[j];
//	}
	f->timeUpdate(&Xa[0], &Xa[S*L], &Xa[0], u, dt, L);//即调用navUkfTimeUpdate()函数  S*L=17*41，即Xa所指向的矩阵的第17行的第一个值

	// sum weighted resultant sigma points to create estimated state//对sigma点集进行加权处理，得到一步预测状态x(k/k-1)
	...
	for (i = 0; i < S; i++) {
		...
		for (j = 1; j < L; j++)
			x[i] += Xa[rOffset + j] * f->wim;//例:x[0]=x[0x41]*f->w0m+(x[0x41+1]+x[0x41+2]+...+x[0x41+40])*f->wim
	}

	// update state covariance//得到一步预测方差矩阵的QR分解因子Sx(k/k-1),也可称为状态协方差矩阵?
	for (i = 0; i < S; i++) {
		int rOffset = i*(S+V)*2;
	...
	qrDecompositionT_f32(&f->qrTempS, NULL, &f->SxT);   // with transposition//得到一个QR的分解因子?
	arm_mat_trans_f32(&f->SxT, &f->Sx);//转置
}
//SRCDKFMeasurementUpdate_t *measurementUpdate定义了一个函数指针measurementUpdate 
//函数指针的一个重要用途就是把函数的址作为参数传递到其它函数
void srcdkfMeasurementUpdate(srcdkf_t *f, float32_t *u, float32_t *ym, int M, int N, float32_t *noise, SRCDKFMeasurementUpdate_t *measurementUpdate) {
	int S = f->S;				// number of states//状态的数量 S为17
	float32_t *Xa = f->Xa.pData;			// sigma point//sigma点
	float32_t *xIn = f->xIn;			// callback buffer
	float32_t *xNoise = f->xNoise;		// callback buffer
	float32_t *xOut = f->xOut;			// callback buffer
	float32_t *Y = f->Y.pData;			// measurements from sigma points
	float32_t *y = f->y.pData;			// measurement estimate//测量预测值
	float32_t *Sn = f->Sn.pData;			// observation noise covariance//测量噪声协方差
	...
	// Y = h(Xa, Xn)
	for (i = 0; i < L; i++) {//这里L这41，S为17
		for (j = 0; j < S; j++)
			xIn[j] = Xa[j*L + i];//xIn[17]里的17个数用Xa所指向的矩阵的每一列(前17个数)去赋值

		for (j = 0; j < N; j++)
			xNoise[j] = Xa[(S+j)*L + i];//xNoise[N]里的N个数用Xa所指向的矩阵的第18行开始的每一列去赋值

		measurementUpdate(u, xIn, xNoise, xOut);

		for (j = 0; j < M; j++)
			Y[j*L + i] = xOut[j];//对Y矩阵的所有元素赋值，Y的每一列元素都相同，是xOut[]
	}

	// sum weighted resultant sigma points to create estimated measurement//对sigma点加权求和得到测量预测值
	f->w0m = (f->hh - (float32_t)(S+N)) / f->hh;
	for (i = 0; i < M; i++) {
		int rOffset = i*L;

		y[i] = Y[rOffset + 0] * f->w0m;//用Y矩阵的第1列的3个值*f->w0m去赋值y[i]

		for (j = 1; j < L; j++)
			y[i] += Y[rOffset + j] * f->wim;//y[i]的值用上面的值累加上第2列到第41列的值*f->wim
	}
	...
	for (i = 0; i < M; i++)
		inov[i] = ym[i] - y[i];
	arm_mat_mult_f32(&f->K, &f->inov, &f->xUpdate);//两个矩阵相乘: K矩阵乘以inov矩阵得到xUpdate矩阵

	for (i = 0; i < S; i++)
		x[i] += xUpdate[i];//这里应该是会更新所有的状态，但重点更新相关的状态值
