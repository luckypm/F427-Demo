...
//          sens = 6442352640;
	...
	//MS5611与MS5607的off与sens计算有点不同，上面是MS5611的，下面是5607的
	  off = ((int64_t)ms5611Data.p[2]<<17) + (((int64_t)dT * ms5611Data.p[4])>>6);
      if (off < -17179344900)
          off = -17179344900;
      else if (off > 25769410560)
          off = 25769410560;

      sens = ((int64_t)ms5611Data.p[1]<<16) + (((int64_t)dT * ms5611Data.p[3])>>7);
      if (sens < -8589672450)
          sens = -8589672450;
      else if (sens > 12884705280)
          sens = 12884705280;
...
//	cs = digitalInit(GPIOD, GPIO_Pin_10, 1);
//	digitalHi(cs);
}
...
uint8_t ms5611Init(void) {
    ...
    utilFilterInit(&ms5611Data.tempFilter, (1.0f / 13.0f), DIMU_TEMP_TAU, 20);
