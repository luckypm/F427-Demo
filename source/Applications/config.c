...
const char *configParameterStrings[] = {// 这个是用来向外传递参数名字的
    "CONFIG_VERSION",
    "RADIO_TYPE",
    "RADIO_SETUP",
    "RADIO_THRO_CH",
...
uint8_t configFlashWrite(void) {
    ...
    if (recs) {
        ...
        if (ret) {
            ...
            ret = flashAddress(flashStartAddr(), (uint32_t *)recs, CONFIG_NUM_PARAMS*sizeof(configRec_t)/sizeof(uint32_t));//这里的len一定是个整数，因为configRec_t结构体是20字节
        }
...
//    else if (ver >= p[CONFIG_VERSION]) //没必要再读一遍了
//	configFlashRead();
